...
var flseDebug = {
  ...
  warn: (input) => {
    console.warn(`FLSE: ${input}`)
  },
  error: (input) => {
    console.error(`FLSE: ${input}`) 
  },
  debug: (input) => {
    console.debug(`FLSE: ${input}`) 
  }
};
try {
  flsestrings;
...
function gatherImports(ft = 0) {
  ...
  for (const importDec of ebimports) {
    if (
      ...
      if (type == null) {
        flseDebug.error(
          `The import for "${importID}" could not be completed.\nThere is no type.`
        );
  ...
  for (const importDec of ebdefine) {
    if (
      ...
      if (type == "package") {
        imports[name] = {
          type: type,
          contents: new Function("environment", importDec.innerHTML),
        };
        importDec.setAttribute("registered", "registered");
        incrementGotCounter(ft);
      }
...
function checkModule(moduleData, moduleName) {
  ...
  if (dataLower.includes("eval")) {
    flseDebug.warn(
      `The module "${moduleName}" may contain potentially harmful code that may pose security risks to this page and any data transferred between it.
  ...
  if (
    ...
    flseDebug.warn(
      `The module "${moduleName}" contains a direct reference to an element that could potentially be used to phish data outbound this page.
  ...
  if (
    ...
    flseDebug.warn(
      `The module "${moduleName}" contains an injection to inline or external scripts that could be used to potentially phish data outbound this page.
  ...
  if (dataLower.includes("fetch") || dataLower.includes("xmlhttprequest")) {
    flseDebug.warn(
      `The module "${moduleName}" may make requests to external sources, which could be used to phish data outbound this page.
  ...
  if (safe == 0) {
    flseDebug.error(
      "There are critical messages being displayed in the Warnings log."
    );
  }
}
